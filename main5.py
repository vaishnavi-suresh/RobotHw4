# main5.py
# bascically main5 but with 5 steps 
...
async def moveToPos(base, slam, x, y, theta):
    ...
    toMove = (target_angle - currTheta + 180) % 360 - 175  # Keeping drift compensation
    print(f'moving to angle: {target_angle}')
    dist = getDist(currX, currY, x, y)
    
    # Changed to 5 steps as recommended
    for i in range(5):
        await moveAngle(base, slam, toMove, target_angle)
        await base.move_straight(int(dist/5), 400)  # Divide distance by 5 steps
