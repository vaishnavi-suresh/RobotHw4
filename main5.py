...
async def moveAngle(base, slam, toMove, target_angle):
    ...
    max_adjustments = 10  # Prevent too many adjustments

    while np.abs(toMove) > 2 and adjustment_iterations < max_adjustments:
        spin_amount = np.clip(toMove / 2, -30, 30)  # Limit spin to prevent overshooting
        await base.spin(spin_amount, 45)
        ...
        await asyncio.sleep(0.1)  # Allow time for the rover to physically adjust

    if adjustment_iterations >= max_adjustments:
        print("Maximum angle adjustments reached. Proceeding with current orientation.")
